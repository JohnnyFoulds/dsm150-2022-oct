...
def get_model(input_shape:int,
    ...
    input_layer = layers.Input(shape=(input_shape,), name='features')
    input_layer_heatmap = layers.Input(shape=heatmap_shape, name='heatmap')
    ...
    model_output = layers.Dense(output_shape, activation='sigmoid')(concat)

    # create the model
    model = k.Model(inputs=[input_layer_heatmap, input_layer], outputs=model_output)

    # compile the model
    if compile_model and (optimizer is not None) and (loss is not None) and (metrics is not None):
        mm.compile_model(
            model=model,

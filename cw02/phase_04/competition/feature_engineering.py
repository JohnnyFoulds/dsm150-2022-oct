...
import competition.data_preparation as dp
...
def add_numeric_column_features(features:pd.DataFrame,
                                ...
                                max_sum:Optional[int]=None) -> pd.DataFrame:
    """
    ...
    df_result = X \
        .groupby(['session_id', 'level_group']) \
        .agg({column: ['sum', 'max', 'min', 'mean', mode, 'std']}) \
        .reset_index()
    ...
    metric_list = ['max', 'min', 'mean', 'mode', 'std']
    for metric in metric_list:
        current_column = f'{column}_{metric}'
        df_result[current_column] = \
            (df_result[current_column] - min_value) / \
            (max_value - min_value)

    # normalize the sum value
    if min_sum is not None and max_sum is not None:
        current_column = f'{column}_sum'

        # clip the values
        df_result[current_column] = \
            df_result[current_column].clip(min_sum, max_sum)

        # normalize
        df_result[current_column] = \
            (df_result[current_column] - min_sum) / \
            (max_sum - min_sum)
    else:
        logging.warning(f'No min/max sum values provided for {column}')
...
def add_numeric_features(features:pd.DataFrame,
                         X: pd.DataFrame,
                         columns:Dict=dp.NUMERIC_COLUMNS) -> pd.DataFrame:
    """
    Add the numeric feature columns.
    """
    df_result = features
    for column, values in columns.items():
        df_result = add_numeric_column_features(
            features=df_result,
            X=X,
            column=column,
            min_value=values['min'],
            max_value=values['max'],
            min_sum=values.get('min_sum', None),
            max_sum=values.get('max_sum', None))
...
def plot_numeric_features(df_features:pd.DataFrame,
    ...
    """
    metric_list = ['sum', 'max', 'min', 'mean', 'mode', 'std']

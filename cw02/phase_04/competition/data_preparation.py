...
def prepare_main_dataset(data : pd.DataFrame,
                         clipping_values:Dict=NUMERIC_COLUMNS) -> pd.DataFrame:
    """
    ...
    """
    empty_columns = ['fullscreen', 'hq', 'music']
    ...
    # clip the numeric columns
    for column, values in clipping_values.items():
        df_main[column] = df_main[column].clip(
